...
<Warning>
  The Compatibility API is **disabled by default** and is enabled per merchant
  on request. Contact support to turn it on for your merchant account. Until
  then every compatibility route rejects your requests: HTTP `403` for
  Cryptomus, Heleket, NowPayments and CoinPayments v2, and an `error` envelope
  for CoinPayments legacy v1 (`api.php`). The native API
  (`https://api.pawpayments.com/api/v2`) is always available.
</Warning>
...
Use the same Paw merchant `api_key` for request signing. Cryptomus and Heleket SDKs send the Paw merchant ID as the `merchant` header and sign with `api_key`. NowPayments SDKs continue to send `x-api-key`. CoinPayments SDKs send the Paw merchant ID as the public `key`/client id and sign with `api_key` (HMAC-SHA512 for legacy, HMAC-SHA256 for v2).
...
Set the public key to your Paw merchant ID and the private key to your `api_key`; the `key` POST field and the `HMAC` header (HMAC-SHA512 of the raw body) keep their format.
...
- Pass the base URL to the client constructor — **without** a trailing path:
  ...
  # client_id = Paw merchant ID, client_secret = Paw api_key
