...
|---|---|---|
| `createInvoice(array $params): array` | `POST /api/v2/invoices` | Returns the created invoice including `payment_url` and `order_id`. The checkout id is not returned separately — keep `payment_url` if you need the link later. |
