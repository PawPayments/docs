...
| `X-Paw-Sandbox: true` | Present only on sandbox deliveries. |


## Payload

The payload is the invoice shape returned by `GET /api/v2/invoices/{id}`
(see the API Reference for the canonical schema), plus `external_id`, which the
read endpoints withhold. Authenticity is established exclusively by the
`X-Paw-Signature` header — there is no `signature` field inside the JSON body.
...
| `external_id` | Public checkout id (UUID) used in the hosted-checkout URL. Delivered here and nowhere else: it is also the credential a payer presents to refund their own flagged payment, so `GET /api/v2/invoices` and `GET /api/v2/invoices/{id}` do not return it. Keep the `payment_url` from the create response if you need the link again. |
...
| `fiat_amount` / `initial_fiat_amount` | What the deposit was worth when it was credited (live rate, net of commission) / the amount the invoice was issued for. On a settled invoice `fiat_amount` can land a fraction under `initial_fiat_amount` — settle against `initial_fiat_amount`, or the larger of the two on `paid_over`. |
...
Sandbox deliveries are identical in shape, with two markers: the payload carries
`"sandbox": true` and the request carries `X-Paw-Sandbox: true`. The signature
is keyed with the **sandbox** API key, so a receiver verifying against the live
key will reject a sandbox webhook — verify with whichever key that environment
is configured with. See [Sandbox](/sandbox) for driving invoices through their
lifecycle by hand.
